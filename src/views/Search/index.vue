<template>
	<div class="search">
		...
		<div
			...
			v-else-if="suggest.length > 0 && searchResults.length === 0"
		>
			<van-cell
				icon="search"
		...
		<div
			...
			"
		>
			<van-cell @click="clickSearch(searchText)"
				>搜索”{{ searchText }}“</van-cell
			>
		</div>
		<!--  搜索结果  -->
		<div class="search-results" v-else-if="searchResults.length !== 0">
			...
			>
				<div
					...
					:key="item.id"
				>
					<div class="item-left van-ellipsis">
						...
						<div class="song-tip van-ellipsis" v-if="item.alia">
							{{ item.alia[0] }}
						</div>
					</div>
					<div class="item-right">
						<van-icon
							class-prefix="iconfont icon-youcecaidan"
							size="0.54rem"
						></van-icon>
					</div>
				</div>
...
export default {
	...
	methods: {
		...
		getSearchSuggest: debounce(async function(value) {
			...
			// 替换的 字符串
			let replaceString =
				'<span style="color: #e04439" class="search-value">' +
				value +
				'</span>';
		...
		async search(text, length) {
			...
			try {
				const { data } = await this.$axios(
					`/cloudsearch?keywords=${text}&type=1&offset=${length || 0}`
				);
